...
const IssueInfo = () => {
    ...
    ;(async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/issues/getIssueById/${id}`,
          {
            headers: {
  ...
  return (
    <div className="fixed flex flex-col  top-0 bottom-0 w-[30%] bg-bgsec border-l-[0.5px] border-l-gray-600 pt-4 right-0">
      ...
      <div className="flex flex-1 text-[12px]">
        <div className="w-full border-r-[0.5px] border-r-gray-600 flex flex-col">
          ...
          <div className="flex flex-col gap-2  px-2 py-4">
            ...
            <p className="text-txtprm">
              <TicketGrid tickets={issue.tickets} isDirected={true}/>
            </p>
          </div>
        </div>
